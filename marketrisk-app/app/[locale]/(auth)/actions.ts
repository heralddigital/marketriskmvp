...
export async function signIn(formData: FormData): Promise<AuthResult> {
  ...
  try {
    ...
    if (error) {
      ...
      if (error.message.includes('fetch failed') || error.status === 0) {
        return { 
          error: 'Nu s-a putut conecta la server. Verifică conexiunea la internet și configurația Supabase.' 
        }
      }
      
      return { error: error.message || 'Email sau parolă incorectă' }
    }
  } catch (err: any) {
    console.error('Sign in error:', err)
    if (err.message?.includes('Missing Supabase environment variables')) {
      return { error: 'Configurație Supabase lipsă. Contactează administratorul.' }
    }
    return { error: 'A apărut o eroare la autentificare. Te rugăm să încerci din nou.' }
  }
