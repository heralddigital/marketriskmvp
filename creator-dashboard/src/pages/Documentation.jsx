...
export default function DocumentationPage({ onNavigate, initialSection }) {
  const [activeSection, setActiveSection] = React.useState(initialSection || 'overview')
  const [addedFeatures, setAddedFeatures] = React.useState(() => {
    try {
      return JSON.parse(localStorage.getItem('addedFeatures') || '[]')
    } catch {
      return []
    }
  })
  ...
  React.useEffect(() => {
    // Update features when localStorage changes
    const handleStorageChange = () => {
      try {
        setAddedFeatures(JSON.parse(localStorage.getItem('addedFeatures') || '[]'))
      } catch {
        setAddedFeatures([])
      }
    }
    ...
    window.addEventListener('addedFeature', handleStorageChange)
    // Check on mount
    handleStorageChange()
    return () => {
      window.removeEventListener('storage', handleStorageChange)
      window.removeEventListener('addedFeature', handleStorageChange)
    }
  }, [])
  ...
  const sections = [
    ...
    { id: 'features', label: 'Added Features' },
    { id: 'roadmap', label: 'Roadmap' },
  ]

  const visibleTasks = ROADMAP_TASKS.filter((t) => t.visible)

  // Scroll to features section if initialSection is 'features'
  React.useEffect(() => {
  ...
  return (
    <>
      <SEO 
        title="Documentation - marketrisk API & Algorithm Guide"
        description="Comprehensive documentation for marketrisk API, risk scoring algorithm, and integration guides for developers and businesses."
        keywords="marketrisk API, risk scoring algorithm, credit risk documentation, API integration, MRCS algorithm"
      />
  ...
  "secret": "your_webhook_secret"
        ...
        <section id="features" className="scroll-mt-8">
          <div className="mb-6">
            ...
            <p className="text-text-secondary mb-6">
              Recently completed features that have been added to the platform. These features are now available for all users.
            </p>
          </div>

          {addedFeatures.length === 0 ? (
            <div className="bg-white border border-border-subtle p-8 rounded-xl text-center">
              <p className="text-text-secondary">No features have been added yet. Complete feature tasks in the Admin Dashboard to see them here.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {addedFeatures.map((feature) => (
                <div key={feature.id} className="bg-white border border-border-subtle p-6 rounded-xl hover:shadow-sm transition-all duration-normal">
                  <div className="flex items-start justify-between mb-3">
                    <h3 className="text-lg font-medium text-text-primary">{feature.title}</h3>
                    <span className="px-2 py-1 bg-brand-mughal-green/10 text-brand-mughal-green rounded-full text-xs font-medium">
                      Added
                    </span>
                  </div>
                  ...
                  {feature.benefits && (
                    <div className="flex items-start gap-2 mb-4">
                      ...
                      <span className="text-xs text-text-secondary">{feature.benefits}</span>
                    </div>
                  )}
                  {feature.addedDate && (
                    <p className="text-xs text-text-muted">
                      Added on {new Date(feature.addedDate).toLocaleDateString('en-GB', { 
                        day: 'numeric', 
                        month: 'long', 
                        year: 'numeric' 
                      })}
